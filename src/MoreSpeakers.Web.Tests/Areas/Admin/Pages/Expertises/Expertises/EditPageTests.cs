...
public class EditPageTests
{
    ...
    {
        typeof(EditModel).GetCustomAttributes(typeof(AuthorizeAttribute), inherit: true)
    ...
    [Fact]
    public async Task OnGetAsync_should_load_entity_or_redirect_when_not_found()
    {
        var manager = new Mock<IExpertiseManager>();
        manager.Setup(m => m.GetAsync(1)).ReturnsAsync(new Expertise { Id = 1, Name = "X", ExpertiseCategoryId = 2 });
        ...
        var page2 = new EditModel(manager.Object, logger.Object) { Id = 9 };
        manager.Setup(m => m.GetAsync(9)).ReturnsAsync((Expertise?)null);
